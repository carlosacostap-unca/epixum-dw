                ...
                {partialExam.turns && partialExam.turns.length > 0 && (
                  <div className="mt-4 grid gap-2">
                    {partialExam.turns.map((turn) => (
                      <div key={turn.id} className="rounded-md border border-zinc-200 px-3 py-2 text-sm text-zinc-600 dark:border-zinc-800 dark:text-zinc-300">
                        <span className="font-medium text-zinc-900 dark:text-zinc-100">{turn.name}</span>
                        <span className="ml-2">
                          <FormattedDate date={turn.startsAt} locale="es-AR" showTime={true} /> - <FormattedDate date={turn.endsAt} locale="es-AR" showTime={true} />
                        </span>
                      </div>
                    ))}
                  </div>
