        ...
        {assignment.type === 'file_upload' && assignment.aiPrompt && (
          <div className="mb-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg max-w-3xl">
            <button 
              ...
              className="w-full flex items-center justify-between text-sm font-bold text-blue-800 dark:text-blue-300 mb-2 focus:outline-none group"
            >
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                Prompt de Preevaluación (IA)
              </div>
              <svg className={`w-4 h-4 transition-transform duration-200 ${isPromptExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </button>
            {isPromptExpanded && (
              <p className="text-sm text-blue-900 dark:text-blue-200 whitespace-pre-wrap mt-3 pt-3 border-t border-blue-200/50 dark:border-blue-800/50 animate-in fade-in slide-in-from-top-2 duration-200">
                {assignment.aiPrompt}
              </p>
            )}
          </div>
